		...
		class Mem {
			public:
				...
				inline byte_t RB(dword_t addr) const
				{
					...
					for (std::vector<AHandler*>::const_iterator it= blocks.begin(); it != blocks.end(); ++it) {
				...
				inline word_t RW(dword_t addr) const
				{
					...
					for (std::vector<AHandler*>::const_iterator it= blocks.begin(); it != blocks.end(); ++it) {
						if ( (*it)->Begin() <= addr && ((*it)->End() > addr)) {
							word_t tmp = ((*it)->RB(addr+1) << 8); // We must enforce a proper order of reading
				...
				inline void WB(dword_t addr, byte_t val) {
					...
					// Search the apropiated block
					for (std::vector<AHandler*>::iterator it= blocks.begin(); it != blocks.end(); ++it) {
						if ( (*it)->Begin() <= addr	&& ((*it)->End() > addr) ){
							(*it)->WB(addr, val);
							return;
						}
				...
				{
					// TODO Use other data structure to contains the blocks
					for (std::vector<AHandler*>::const_iterator it= iblocks.begin(); it != iblocks.end(); ++it) {
						blocks.push_back(*it);
					}
					return true;
				}
