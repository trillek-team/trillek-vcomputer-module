...
static const dword_t INT_MSG[] = {
    /// Interrupt message value
    0x0000005A,
    0x0000105A,
    0x0000205A,
    0x0000305A
...
static const dword_t BASE_ADDR[] = {
    /// VRAM base address
    0xFF0A0000,
    0xFF0B0000,
    0xFF0C0000,
 ...
 * @param textmode True to indicate if we are using a Text Mode or a Graphics
 **mode
...
public:
    ...
    byte_t DevClass() const {
        return 0x0E;
    } // Graphics device

    word_t Builder() const {
        return 0x0000;
    } // Generic builder
    ...
    word_t DevVer() const {
        return 0x0000;
    } // Ver 0 -> CDA base standard

    DeviceFlags Flags () const {
        return DeviceFlags::WITH_INTERRUPTS;
    }
     ...
     * @param texture Ptr were write the RGBA texture, must have the correct
     **size
     */
...
protected:
    ...
    byte_t* buffer; /// Visible video ram buffer for grpahics representation of
                    // screen
...
#else // if ( ( BYTE_ORDER != LITTLE_ENDIAN) && !EMSCRIPTEN)
// Little Endian -> ABGR
    0xFF000000, // Black
...
#endif // if ( ( BYTE_ORDER != LITTLE_ENDIAN) && !EMSCRIPTEN)

static const byte_t def_font[256*8] = {
    /// Default font
#include "CDAfont.inc"
};
