...
namespace vm {
  namespace cpu {
    ...
    const DCPU16N_DisInfo DCPU16N_Operand[] = {
      ...
      { "[Y+", "]", true }, { "[Z+", "]", true }, { "[I+", "]", true }, { "[J+", "]", true },
      { "[SP++]", "[--SP]", false }, { "[SP]", "", false }, { "[SP+", "]", true }, { "SP", "", false },
      { "PC", "", false }, { "EX", "", false }, { "[", "]", true }, { "", "", true }
    };
    std::string DisassemblyDCPU16N(const VComputer& vc, dword_t pc) {
      word_t opc = (((word_t)vc.ReadB(pc + 1)) << 8) | (word_t)vc.ReadB(pc);
      word_t opta, optb;
      word_t opa, opb;
#define BUF_SIZE (32)
      char buf[BUF_SIZE] = { 0, };
      ...
      if((opc & 0x001f) != 0 || (opc & 0x03e0) != 0) {
        ...
        if(opta & 0x20) {
          ...
          if(DCPU16N_Operand[opta & 0x1f].fetch) {
            pc += 2;
            opa = (((word_t)vc.ReadB(pc + 1)) << 8) | (word_t)vc.ReadB(pc);
      ...
      if((opc & 0x001f) != 0) {
        ...
        if(optb == 0x18) {
          ...
          if(DCPU16N_Operand[optb & 0x1f].fetch) {
            pc += 2;
            opb = (((word_t)vc.ReadB(pc + 1)) << 8) | (word_t)vc.ReadB(pc);
